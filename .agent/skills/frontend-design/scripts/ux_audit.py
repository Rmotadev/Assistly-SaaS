...
class UXAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        limit = 15 if 'page' in filename.lower() else 7
        if nav_items > limit:
            self.issues.append(f"[Hick's Law] {filename}: {nav_items} nav items (Max {limit})")
    ...
    def audit_directory(self, directory: str) -> None:
        ...
        for root, dirs, files in os.walk(directory):
            dirs[:] = [d for d in dirs if d not in {'node_modules', '.git', 'dist', 'build', '.next', 'coverage', 'out'}]
            for file in files:
